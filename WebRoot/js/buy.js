...
//    			layer.msg(data.message);
  ...
  function loadproducts(){
      ...
      $.post("/buy/getcount.json",param,function(data){
  ...
  function pageSelectCallback(index, jq){
  	...
  	$.post('/buy.json',param,function(data){
  		...
  		if(data.error){
  			...
  			if(PRODUCTLIST_.length>0){
  				...
  				for(var i=0;i<PRODUCTLIST_.length;i++){
  					...
  					html_+="<li>"+
  						   "     <em><a target=\"_blank\" href=\"/buy/buy_show.json?ID="+PRODUCT.id+"&ITEM="+(PRODUCT.product_items_id==null?0:PRODUCT.product_items_id)+"\"><img src=\"http://img.tulingbuy.com"+PRODUCT.img_url+"\" onerror=\"nofind(this)\"/></a></em>"+
