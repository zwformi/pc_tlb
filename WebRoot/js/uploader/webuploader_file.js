$(function () {
    ...
    $.upLoadDefaults.property = {
        ...
        sendurl: null, //发送地址
        filetypes: "doc,docx,xls,xlsx,jpg,png,jpeg,rar,zip,txt", //文件类型
        filesize: "4096", //文件大小
        btntext: "浏览...", //上传按钮的文字
    ...
    $.fn.InitUploader = function (b) {
        var fun = function (parentObj) {
            ...
            if (!p.multiple) {
                p.sendurl += "&DelFilePath=" + parentObj.parent().parent().find("#file_url").val();
            }
            ...
            uploader.on('fileQueued', function (file) {
                //如果是单文件上传，把旧的文件地址传过去
                if (!p.multiple) {
                    uploader.options.formData.DelFilePath = parentObj.prev().val();
                }
                //防止重复创建
