...
<script>
       function attr(key1,value1,key2,value2,key3,value3,key4,value4,key5,value5,key6,value6,key7,value7,key8,value8){
       ...
       this.key8 =key8;this.value8 =value8;
     } 
     var GetAllData=[];  
     var IsNull_Attr_1=false;
</script>
<body>
  ...
  <div class="product_right" style="float:left">
    <h2><span id="p_title">${PRODUCT.name }</span><br/>
      ...
      <div class="proudct_right_price">      	
      	...
      	<li class="proudct_right_price_item">
      		<strong id="sale_price">￥<fmt:formatNumber type="number" value="${PRODUCT.price_new }" pattern="0.00" maxFractionDigits="2"/></strong>
      	</li>      	
      	</ul>
    ...
    <div id="item_content">
	    	...
	    	<c:forEach items="${PRODUCTITEMS}" var="ITEMINFO" varStatus="status">
	    		<script>
	    		       var Attr=new attr("${ITEMINFO.attrName1}","${ITEMINFO.attrVal1}","${ITEMINFO.attrName2}","${ITEMINFO.attrVal2}","${ITEMINFO.attrName3}","${ITEMINFO.attrVal3}","${ITEMINFO.attrName4}","${ITEMINFO.attrVal4}","${ITEMINFO.attrName5}","${ITEMINFO.attrVal5}","${ITEMINFO.attrName6}","${ITEMINFO.attrVal6}","${ITEMINFO.attrName7}","${ITEMINFO.attrVal7}","${ITEMINFO.attrName8}","${ITEMINFO.attrVal8}");                       
                       var Att={"Att":Attr};
                       var Other={};
	    		       Other.product_marketprice="${ITEMINFO.product_marketprice}";
	    		       Other.product_saleprice="${ITEMINFO.product_saleprice}";
	    		       Other.stocks="${ITEMINFO.stocks}";
	    		       Other.product_items_id="${ITEMINFO.product_items_id}";
	    		       Other.itemname=$("#p_title").text();
	    		       var Attr=[Att,Other]; 
	    		       GetAllData.push(Attr);
	    		</script>   		
	    	</c:forEach>	    		
...
&nbsp;库存：<span id="countname" >${PRODUCT.count}</span><input type="hidden" id="count" value="${PRODUCT.count }"/>
  ...
  <div class="product_right" style="float:right;width:190px">
  	  ...
  	  <p style="line-height:30px"><span class="bt_buyshow">
  	  								<span class="iconfont icon-shouji" style="color:#4489FD;display:inline-block;font-size:22px"></span><span class="companynumber">400-826-4755</span></span></p>
