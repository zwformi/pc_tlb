...
<script src="<%=basePath %>js/buy.js?date=<%=new Date()%>"></script>
...
<div class="member_center" style="width:1300px">
 ...
 <div class="buy_tj">
   <h2><span>热卖推荐</span></h2>
   <ul>
   		<c:forEach items="${PRODUCTREDLIST }" var="PRODUCTRED">
   			
		     <li>
		       <img src="http://img.tulingbuy.com${PRODUCTRED.img_url }" onerror="nofind(this)" style="cursor:pointer;" onclick="javascript:window.open('/buy/buy_show.json?ID=${PRODUCTRED.id }&ITEM=${PRODUCTRED.product_items_id}')"/>
		        <h3>${PRODUCTRED.name }</h3>
		        <span><b>￥<fmt:formatNumber type="number" value="${PRODUCTRED.price_new }" pattern="0.00" maxFractionDigits="2"/></b></span>
		        <c:if test="${PRODUCTRED.is_ms==1 }">
		        	<c:if test="${PRODUCTRED.overplus>0 }">
		        		<strong><a href="/buy/buy_show.json?ID=${PRODUCTRED.id }&ITEM=${PRODUCTRED.product_items_id}" product_id="${PRODUCTRED.id }" product_items_id="${PRODUCTRED.product_items_id}">立即秒杀</a></strong>
		        	</c:if>
		        	<c:if test="${PRODUCTRED.overplus<=0 }">
		        		<strong><a href="javascript:void(0);">~抢光了~</a></strong>
		        	</c:if>
		        </c:if>
		        <c:if test="${PRODUCTRED.is_ms==0 }">
		        	<c:if test="${PRODUCTRED.overplus>0 }">
		        		<strong><a class="add_cart" product_id="${PRODUCTRED.id }" product_items_id="${PRODUCTRED.product_items_id}">加入采购车</a></strong>
		        	</c:if>
		        	<c:if test="${PRODUCTRED.overplus<=0 }">
		        		<strong><a href="javascript:void(0);">缺货</a></strong>
		        	</c:if>
		        </c:if>
		     </li>
       </c:forEach>
