...
<!--left-->
 <div class="main_left">
   <dl class="about_menu">
   	 <c:forEach items="${articleList}" var="articles">
     	<dd><a href="<%=basePath %>article/${articles.typeid}-${articles.id }.json" ><i class="fa fa-arrow-circle-o-right"></i> ${articles.title }</a></dd>
     </c:forEach>
 ...
 <div class="main_right">
  <dl class="about_cont">
     <dt><span>图灵买快报</span></dt>
     <dd>
       <c:forEach items="${articleList_dsj}" var="articles">
     	<div class="a_list"><img src="/images/a_left.png" align="absmiddle"/><a href="<%=basePath %>article/gsdsjxq.json?typeid=2&ID=${articles.id }" >${articles.title }</a><span>${articles.create_time_str }</span></div>
       </c:forEach>
     </dd>
  </dl>
 </div>
