...
<!--left-->
 ...
 <div class="member_content" style="min-height:535px;">
     <h3>
       ...
       <a  class="select" href="/find/find_contract.html?page=find_contact">订单合同</a>
...
$(document).ready(function(){
	...
	$(".radius").each(function(){
		...
		$.post('<%=basePath%>order/kt.json',{"order_id":order_id},function(data){
		layer.closeAll('loading');
		if(data.resMsg=="error"){
	    			layer.msg(data.message);
		}else{
				layer.msg("开通成功");
				$this.parent().html("<span class=\"iconfont icon-kuaidi\" style=\"color:#E3393C;display:inline-block\"></span>"+statelist[2]);
			}
			});
	});
...
function loadproducts(){
	//显示加载层
	layer.load();
    $.post("<%=basePath%>find/getcount.json",{"type":"order","order_state":order_state},function(data){
   		if(data.count>0){
...
function pageSelectCallback(index, jq){
	...
	var param ={"type":"order","page":index,"size":pageSize,"order_state":order_state};
	$.post('<%=basePath%>find/paginglist.json',param,function(data){
		...
		if(data.resMsg=="error"){
			...
			var _html='<table class=" table_show1" width="100%" border="0" cellspacing="0" cellpadding="0">'
	           ...
	           +'<tr >'
	            + '<th style="width:300px;"><span>生成时间:'+new Date(list[i].order_time).Format("yyyy-MM-dd hh:mm:ss")+'</span></th>'
	             +'<th style="width:400px;"><span>合同编号:'+list[i].order_number+'</span></th>	'
	             +'<th style="width:150px;">'
			            +' <span class="iconfont icon-dianpu" style="color:#E3393C"></span>&nbsp;'+sourcelist[list[i].order_source+""]+'</th>'	                         
	            +'<th style="width:130px;"><span class="iconfont icon-profile" style="float:left;color:#E3393C"></span> <span style="float:left;width:80px;overflow:hidden;white-space: nowrap;text-overflow: ellipsis;">&nbsp;'+(list[i].shr_xm==null||list[i].shr_xm==""?"未填写":list[i].shr_xm)+'</span></th> '
	           +'</tr>'
	         +' </thead>'
	      + '</table>';
	      _html += '<table class="order_show table_show" width="100%" border="0" cellspacing="0" cellpadding="0">'
	            ...
	            + '<td  style="width:150px;" class="table_right" order_id="'+list[i].id+'">';
	            if(list[i].order_state==0)
	            _html+= '<a style="color:red;" href="<%=basePath%>pay.html?order_number='+list[i].order_number+'">未付款 ,立即支付</a>';
	            else 
	            _html+= '<span class="iconfont icon-kuaidi" style="color:#E3393C;display:inline-block"></span>'+statelist[list[i].order_state]+'';
	            if(list[i].order_state==1 && list[i].order_source==4)
	            _html +='<a class="radius" target="_blank" >开通</a>'
	     	    _html+= ' </td>	'
	      	    + ' <td style="width:200px;">'
	            + '<a style="color:#7fa9ee" href="<%=basePath%>find/find_contract_show.html?order_number='+list[i].order_number+'"><i class="fa fa-file-text-o"></i> 订单详情</a>'
	      		+ ' </td>'
	      		+ '</tr>'       	  
	       		+'</tbody></table></br>';
